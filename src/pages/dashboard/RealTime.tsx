...
const Activities: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <CardContent>
          <ul className="space-y-2 max-h-[520px] overflow-auto">
            {alerts.map((a) => (
              <li
                key={a.id}
                className="flex items-center justify-between p-3 rounded border border-gray-200 dark:border-white/10"
              >
                <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Alerts / Minute</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {Object.entries(perMinute)
              .slice(-12)
              .map(([m, c]) => (
